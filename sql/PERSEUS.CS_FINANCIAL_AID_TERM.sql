...
TRUNCATE TABLE perseus_workday.cs_financial_aid_term;

INSERT INTO perseus_workday.cs_financial_aid_term (
     emplid
    ,acad_career
    ,strm
    ,first_term_flag
...
SELECT a.emplid
    ...
    ,a.strm
    ,CASE
      WHEN a.strm = (MIN(a.strm) OVER(PARTITION BY a.emplid,a.acad_career ROWS BETWEEN unbounded preceding AND unbounded following))
      THEN 'Y' ELSE 'N'
   ...
   END AS fa_type
    ...
    ,a.offer_amount
    ,a.samia_scholars_accept_amount
...
FROM (
    ...
    , SUM(fa.disbursed_amount) AS disbursed_balance,
      sum( case when fa.item_type in ('SAM','SAMIA','SNE','SSAMI') then fa.accept_amount end) samia_scholars_accept_amount
...
COMMIT;
ANALYZE perseus_workday.cs_financial_aid_term;